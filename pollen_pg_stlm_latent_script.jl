...
if (!isfile("output/pollen_latent_fit.jld"))
    BLAS.set_num_threads(32);
    tic = now();
    out = pg_stlm_latent(Y, X, locs, params, priors); 
    toc = now();

    save("output/pollen_latent_fit.jld", "data", out);
    #delete!(out, "runtime"); # remove the runtime which has a corrupted type
    R"saveRDS($out, file = 'output/pollen_latent_fit.RDS', compress = FALSE)";
end
