...
version='5.0'
# model_list = c("matern", "overdispersed", "latent") 
model_list = c("matern", "overdispersed") # latent model not currently fitting
for (model in model_list) {
    ...
    preds <- readRDS(here::here('output', 'pollen', paste0('pollen_', model, '_preds.rds')))
    ...
    map <- ggplot() +
        ...
        geom_polygon(data = dat_lake_shp,
                     ...
                     color = 'black', size = .2)
    map + geom_point(data = locs_grid, aes(x=x, y=y), size = 0.01)
    ...
    for (species_to_plot in unique(dat_pi_mean$species)) {
        ...
        p_mean <- dat_pi_mean %>%
            ...
            ggtitle(substitute(paste("Posterior proportion mean of ", italic(x), " for the ", italic(model_name), " model"), list(x = as.character(species_to_plot),
        ...
        p_sd <- dat_pi_sd %>%
            ...
            facet_wrap(~ time, nrow = 3, labeller = as_labeller(time_vec)) +
            scale_fill_viridis_c() +
            ggtitle(substitute(paste("Posterior proportion sd of ", italic(x), " for the ", italic(model_name), " model"), list(x = as.character(species_to_plot),
                                                                                                                                model_name = as.character(model_name)))) +
            theme_bw(base_size = base_size) +
            theme(axis.text.x = element_blank(),
                  axis.ticks.x=element_blank(),
        ...
        ggsave(p_mean / p_sd,
               file = here::here("figures", paste(model), paste0("predictions-", species_to_plot, "-version-", version, ".png")),
               height = 12,
               width = 16)
    }
}