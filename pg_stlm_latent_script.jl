...
println("Starting pg_stlm_latent_script")
flush(stdout)
...
if (!isfile("output/latent_sim_fit.jld"))
    ...
    tic = now();
    out = pg_stlm_latent(Y, X, locs, params, priors, corr_fun="matern"); # XX minutes for 200 iterations -- can this be sped up more through parallelization?
