println("Starting pollen_pg_stlm_cv_script")
flush(stdout)
...
for k in 1:K
    ...
    out = pg_stlm(Y, X, locs, params, priors, corr_fun="matern", path = "./output/cross-validate/pollen_matern_fit_fold_$k.jld"); 
    ...
    alert("Finished Matern fitting cross-validation fold $k out of $K")

    if (!isfile("./output/cross-validate/pollen_matern_fit_fold_$k.rds"))
        R"saveRDS($out, file = './output/cross-validate/pollen_matern_fit_fold_$k.rds', compress = FALSE)";
    end
end 
