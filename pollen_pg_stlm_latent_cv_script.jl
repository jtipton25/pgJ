println("Starting pollen_pg_stlm_latent_cv_script")
flush(stdout)
...
K = 8
for k in 1:K
    println("Fitting Latent model for fold $k of $K")
    flush(stdout)
    ...
    out = pg_stlm_latent(Y, X, locs, params, priors, corr_fun="matern", path = "./output/cross-validate/pollen_latent_fit_fold_$k.jld"); 
    ...
    alert("Finished Latent fitting cross-validation fold $k out of $K")

    if (!isfile("./output/cross-validate/pollen_latent_fit_fold_$k.rds"))
        R"saveRDS($out, file = './output/cross-validate/pollen_latent_fit_fold_$k.rds', compress = FALSE)";
    end
end 
