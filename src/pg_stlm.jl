...
function pg_stlm(Y, X, locs, params, priors)
    ...
    tau2_save = Array{Float64}(undef, (n_save, J - 1))
    ...
    for k = 1:(params["n_adapt"]+params["n_mcmc"])
        ...
        if (sample_beta)
            for j = 1:(J-1)
                ...
                b = dropdims(
                    sum(tXSigma_inv * eta[:, j, :], dims = 2) + Sigma_beta_inv_mu_beta,
                    dims = 2,
                )
        ...
        if (sample_theta)
            for j = 1:(J-1)
                theta_star = rand(Normal(theta[j], theta_tune[j]), 1)
                ...
                mh1 =
                    ...
                    logpdf(Normal(theta_mean, sqrt(theta_var)), theta_star)[1]
        ...
        if (sample_rho)
            for j = 1:(J-1)
                rho_star = rand(Normal(rho[j], rho_tune[j]), 1)[1]
