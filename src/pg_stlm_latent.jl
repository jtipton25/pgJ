...
function pg_stlm_latent(Y, X, locs, params, priors; corr_fun="exponential", path="./output/pollen/pollen_latent_fit.jld", save_full=false, correct_initial_variance=true)
    ...
    if save_full
        if isfile(path)
            ...
            out = Dict(
                ...
                "psi" => Array{Float64}(undef, (params["n_adapt"] + params["n_mcmc"], N, J - 1, n_time)),
        ...
        if isfile(path)
            ...
            out = Dict(
                ...
                "psi" => Array{Float64}(undef, (n_save, N, J - 1, n_time)),
    ...
    for j in 1:(J-1)
        psi[:, j, 1] = rand(MvNormal(zeros(N), Sigma[j]), 1)
        for t = 2:n_time
            psi[:, j, t] =
                rand(MvNormal(rho[j] * psi[:, j, t-1], Sigma[j]), 1)
        end
    end
