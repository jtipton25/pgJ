...
"""
function predict_pg_stlm(out, X_pred, locs_pred; path="./output/pollen/pollen_matern_predictions.jld", posterior_mean_only=false, n_message = 50, n_save = 50)
    ...
    for k in k_start:n_samples
        ...
        Threads.@threads for j in 1:(J-1)
            ...
            Q_time = diagm(vcat(1, (1 + rho[k, j]^2) * ones(n_time-2), 1)) - rho[k, j] * W_time
