# From AugmentedGaussianProcesses at https://github.com/theogf/AugmentedGaussianProcesses.jl/blob/71a926410de3576a8c32df6a344694d1c86a56df/src/ComplementaryDistributions/polyagamma.jl
...
function rand_truncated_inverse_gaussian(rng::AbstractRNG, z::Real)
    ...
    x = one(z) + pg_t
    if μ > pg_t
        d_exp = Distributions.Exponential()
        while true
            E = rand(rng, d_exp)
            E′ = rand(rng, d_exp)
            while E^2 > 2E′ / pg_t
...
function sample_pg1(rng::AbstractRNG, z::Real)
    ...
    while true
        if r > rand(rng) # sample from truncated exponential
            x = t + rand(rng, Distributions.Exponential()) / K
